...
1. **用户入群检查昵称、bio 和 Premium 状态表情包名称**：读取用户实体，调用 `users.getFullUser` 获取 `about`，交给模型判定。用户名字旁有效的普通自定义 emoji 状态会查询所属表情包的 `title` / `short_name`，与昵称和 bio 一起送审；不是识别表情图案，也不检查聊天消息中的自定义 emoji。已过期状态不查询，收藏礼物状态不在此项范围内。表情包查询失败会重试，不视为审核通过。资料命中广告则踢出。普通直接入群也检查，无需要求用户先私聊 bot。资料查询失败会重试，并先尝试检查可获得的昵称。
   - 新人阶段资料变化重审：处理 `UpdateUserName`、`UpdateUserEmojiStatus`、`UpdateUser`，并在发言时比较完整 User 实体中的昵称、用户名及状态 emoji。只覆盖已知在群、当前入群轮次不足三条 `checked=1` 的成员；名额占满但仍待审也属于此范围，历史导入的认可名额按现有计数规则生效。资料重审不占用或重置发言名额。不做定期轮询；没有收到事件且不发言的用户可能漏检。
   - 同一轮次的资料快照保存在 SQLite `profile_state` 表；与上次成功非广告审核相同的完整资料不重复调用模型。新版任务使旧结论失效；资料封禁前重新获取资料核对，发生变化则取消该次处罚并在新人范围内安排刷新。该核对不是与 Telegram 封禁操作原子执行，最终查询与操作之间仍有很短的竞态窗口。管理员、豁免和退群检查继续生效。
2. **新人前三条可审核发言检查**：文字消息先登记再异步调用模型；默认删除新人发送的图片、视频、语音、贴纸、文件等无法完整审核的媒体，带说明文字也删除，不占用三条名额。三条文字消息完成非广告判定后才允许发送媒体。计数和审核状态保存在 SQLite；原三条消息被编辑仍复查。退出后重新加入重新计数，重复入群更新不会重复重置。
   - 发言审核同时提取回复头的引用文字、隐藏链接、来源标签。跨群回复没有引用文字或链接时，尝试读取被回复的单条原消息，不递归追踪回复、不扫描历史。取得的内容以 `reply_context` 与当前正文分开送审和记录日志；新人跨群回复作为强广告信号，重点检查正文仅为表情或附和、实际通过回复卡片推广的情况；明确举报、反诈提醒、反驳或具体正常讨论仍结合上下文判断，不因跨群本身无条件封禁。跨群原消息无权限、已删除或仅有无法审核的媒体时，任务重试，不算审核通过，不因无法读取而处罚。
...
- `data/telegram.session` 存放 MTProto 会话及实体访问资料；`data/moderation.sqlite3` 保存成员轮次、前三条 ID、任务、资料快照和日志。待审/失败任务保留文本以便重试；完成任务清空 payload，保留任务元数据和去重指纹。控制台 `review_input` 记录实际送审内容，`review_result` 记录判定、置信度、理由和原文证据，并用任务 ID / 版本关联；JSON 转义换行以避免伪造日志行。audit 保存判定、理由及证据，完成任务的原始输入仍只保留在进程日志中。日志可能包含昵称、bio、状态表情包名称和消息正文，应妥善保管。数据需要按你的保留周期维护。
- 新安装不能补查部署前的全部聊天历史，长时间离线也可能错过历史更新。不会持续扫描用户之后修改的简介；资料变化重审依赖收到的更新及发言中可见的变化，不能保证独立的 bio 修改、表情包作者改名会被及时发现。
...
配置 `AUDIT_CHANNEL_ID=@频道用户名`（推荐）或已知实体的 `-100…` 频道 ID，并授予 bot 发布频道消息的权限。
`AUDIT_CHANNEL_MODE=flagged` 默认只发布广告判定（包括未达阈值）及媒体/群外 bot 清理；`all` 发布全部有效审核结论。
留空频道配置则关闭。目标必须是广播频道，不能同时列入受审群；私有频道数字 ID 需要 bot 的 session 已有访问实体。

记录使用 Telethon 1.45.0 的 `InputRichMessageHTML` 原生表格，展示群/用户/消息 ID、任务版本、判定、证据、模型自评分、阈值及处置状态。
展示用的用户文本全部转义并禁用自动链接识别；过长字段标注截断。不转发附件，不发布 LLM 原始响应、思考过程或配置凭据。
已发布记录在删除/封禁/解除封禁结果变化时编辑，旧源任务版本标记失效；已发生的处置不被事后资料更新撤销。
`DRY_RUN=true` 仍会实际发布频道日志，但处置显示“仅模拟”。

SQLite 自动新增 `channel_reports` 表，保存发布快照、目标频道、随机请求 ID、频道消息 ID、版本、重试时间和错误。
独立 worker 负责发布，权限问题或限流不会占用审核 worker；遵守 FloodWait，达到 JOB_MAX_ATTEMPTS 后留待管理员处理。
修复权限后可沿用 `--retry-failed` 重新排队失败发布。正常重试复用 random_id；Telegram 去重窗口及远端/本地提交非原子性使极端故障下无法承诺绝对恰好一次。
修改目标频道不会将旧频道待发记录自动转发到新频道；禁用发布也不会删除已有记录。
新功能不回放升级前的 audit 历史。频道记录和数据库快照含用户资料，应自行维护保留周期。

部署后用测试群及普通测试账号验证，先用 DRY_RUN 检查发布和排版，再验证正常模式的真实处置状态。
单元测试验证请求类型、转义、状态更新和队列恢复；未授权的真实频道不发送测试消息。
